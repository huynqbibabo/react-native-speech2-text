...
import com.speech2text.speech_service.SpeechEvent;
import com.speech2text.speech_service.SpeechService;
import com.speech2text.voice_recorder.VoiceEvent;
import com.speech2text.voice_recorder.VoiceRecorder;

import io.reactivex.disposables.CompositeDisposable;

public class Speech2TextModule extends ReactContextBaseJavaModule {
    private static final String TAG = "Speech2Text";
    ...
    public Speech2TextModule(ReactApplicationContext reactContext) {
        super(reactContext);
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            compositeDisposable.add(
                    speechService.getSpeechEventObservable()
                            .subscribe(Speech2TextModule.this::handleSpeechEvent)
            );
            compositeDisposable.add(
                    speechService.getSpeechErrorEventObservable()
                            .doAfterNext(event -> stop())
                            .subscribe(Speech2TextModule.this::handleErrorEvent)
            );
            compositeDisposable.add(
                    voiceRecorder.getVoiceEventObservable()
                            .subscribe(Speech2TextModule.this::handleVoiceEvent)
            );
            compositeDisposable.add(
